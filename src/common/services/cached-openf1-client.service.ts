...
export class CachedOpenF1ClientService {
  ...
  async fetchDrivers(params: DriversQueryParams): Promise<OpenF1Driver[]> {
    const cacheKey = this.cacheService.generateSessionKey(params.session_key, 'drivers');
    
    const cachedData = await this.cacheService.get<OpenF1Driver[]>(cacheKey);
    if (cachedData) {
      this.logger.debug(`Returning cached drivers for session ${params.session_key}`);
      return cachedData;
    }

    this.logger.debug(`Fetching drivers from API for session ${params.session_key}`);
  ...
  async fetchCarData(params: CarDataQueryParams): Promise<OpenF1CarData[]> {
    ...
    if (cachedData) {
      this.logger.debug(`Returning cached car data for driver ${params.driver_number}`);
      return cachedData;
    }

    this.logger.debug(`Fetching car data from API for driver ${params.driver_number}`);
  ...
  async fetchIntervals(params: IntervalsQueryParams): Promise<OpenF1Interval[]> {
    const cacheKey = this.cacheService.generateSessionKey(params.session_key, 'intervals');
    
    const cachedData = await this.cacheService.get<OpenF1Interval[]>(cacheKey);
    if (cachedData) {
      this.logger.debug(`Returning cached intervals for session ${params.session_key}`);
      return cachedData;
    }

    this.logger.debug(`Fetching intervals from API for session ${params.session_key}`);
  ...
  async fetchRaceControl(params: RaceControlQueryParams): Promise<OpenF1RaceControl[]> {
    ...
    const cachedData = await this.cacheService.get<OpenF1RaceControl[]>(cacheKey);
    if (cachedData) {
  ...
  async fetchStints(params: StintsQueryParams): Promise<OpenF1Stint[]> {
    const cacheKey = this.cacheService.generateSessionKey(params.session_key, 'stints');
    
    const cachedData = await this.cacheService.get<OpenF1Stint[]>(cacheKey);
    if (cachedData) {
      this.logger.debug(`Returning cached stints for session ${params.session_key}`);
      return cachedData;
    }

    this.logger.debug(`Fetching stints from API for session ${params.session_key}`);
  ...
  async preloadReplayData(sessionKey: number): Promise<{
    ...
    this.logger.log(`Preloaded replay data: ${drivers.length} drivers, ${laps.length} laps, ${intervals.length} intervals, ${stints.length} stints`);
  ...
  private generateLapsCacheKey(params: LapsQueryParams): string {
    if (params.lap_number) {
      return this.cacheService.generateLapKey(params.session_key, params.lap_number);
    }
    
    if (params.driver_number) {
      return this.cacheService.generateDriverKey(params.session_key, params.driver_number, 'laps');
    }
