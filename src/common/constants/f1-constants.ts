...
export const F1_CONSTANTS = {
  ...
  RACE_CONTROL: {
     ...
     */
    DRS_KEYWORDS: [
      'DRS', 'DRAG REDUCTION SYSTEM', 'DRS ENABLED', 'DRS DISABLED'
    ] as const,
  },
...
export type SpeedCategory = typeof F1_CONSTANTS.PERFORMANCE.SPEED_RANGES[number]['category'];
export type LogContext = typeof F1_CONSTANTS.LOGGING.CONTEXTS[keyof typeof F1_CONSTANTS.LOGGING.CONTEXTS];
...
export class F1ConstantsHelper {
  ...
  static isValidLapTime(lapTime: number): boolean {
    return lapTime > 0 && lapTime <= F1_CONSTANTS.VALIDATION.MAX_VALUES.LAP_TIME;
  }
  ...
  static hasSufficientDataForConsistency(dataPoints: number): boolean {
    return dataPoints >= F1_CONSTANTS.VALIDATION.MIN_DATA_POINTS.CONSISTENCY_ANALYSIS;
  }
}