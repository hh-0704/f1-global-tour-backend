...
export class LapsService {
  ...
  async getSpecificLap(sessionKey: number, lapNumber: number) {
    try {
      ...
      const lapByDriver = transformedLaps.reduce((acc, lap) => {
  ...
  async getDriverLaps(sessionKey: number, driverNumber: number, lapNumber?: number) {
    try {
      ...
      this.logger.log(`Retrieved ${progressiveLaps.length} laps for driver ${driverNumber}`);
  ...
  private transformLapData(lap: any) {
    return {
      ...
      isPitLane: this.detectPitLane(lap.segments_sector_1, lap.segments_sector_2, lap.segments_sector_3),
      segments: {
  ...
  private detectPitLane(sector1: number[], sector2: number[], sector3: number[]) {
    const allSegments = [...(sector1 || []), ...(sector2 || []), ...(sector3 || [])];
  ...
  private addProgressiveAnalysis(laps: any[]) {
    return laps.map((lap, index) => {
      ...
      const completedPreviousLaps = previousLaps.filter(l => !l.isDNF && l.lapTime);
      
      let bestLapSoFar: number | null = null;
      let averageLapSoFar: number | null = null;
      
      if (completedPreviousLaps.length > 0) {
        const lapTimes = completedPreviousLaps.map(l => l.lapTime).filter((time): time is number => time !== null);
        if (lapTimes.length > 0) {
          bestLapSoFar = Math.min(...lapTimes);
          averageLapSoFar = lapTimes.reduce((sum, time) => sum + time, 0) / lapTimes.length;
        }
      }
      ...
      return {
        ...
        progressive: {
          bestLapSoFar,
          averageLapSoFar,
          isPersonalBest: lap.lapTime && bestLapSoFar ? lap.lapTime <= bestLapSoFar : false,
  ...
  private calculateDriverStats(laps: any[]) {
    ...
    const driverStats = Array.from(driverMap.values()).map(driver => {
      if (driver.lapTimes.length > 0) {
        ...
        driver.averageLapTime = driver.lapTimes.reduce((sum, time) => sum + time, 0) / driver.lapTimes.length;
      }
  ...
  private analyzeLapProgression(laps: any[]) {
    const lapNumbers = [...new Set(laps.map(lap => lap.lapNumber))].sort((a, b) => a - b);
    
    return lapNumbers.map(lapNumber => {
      const lapsInThisNumber = laps.filter(lap => lap.lapNumber === lapNumber);
      const completedLaps = lapsInThisNumber.filter(lap => !lap.isDNF && lap.lapTime);
      ...
      return {
        ...
        averageTime: lapTimes.reduce((sum, time) => sum + time, 0) / lapTimes.length,
