...
export class IntervalsService {
  ...
  async getSessionIntervals(sessionKey: number, date?: string) {
    try {
      ...
      const transformedIntervals = intervals.map((interval) =>
        this.transformIntervalData(interval),
      );

      this.logger.log(
        `Retrieved ${transformedIntervals.length} intervals for session ${sessionKey}`,
      );
  ...
  async getLiveIntervals(sessionKey: number) {
    try {
      ...
      this.logger.log(
        `Retrieved live intervals for ${positionedIntervals.length} drivers`,
      );
      return {
        sessionKey,
        ...
        .map((interval, index) => {
          ...
          return {
            ...
            gapToPrevious:
              ...
              previousDriver.gapToLeader !== null
  ...
  async getDriverGaps(sessionKey: number, driverNumber: number) {
    try {
      this.logger.debug(
        `Fetching gaps for driver ${driverNumber} in session ${sessionKey}`,
      );
      const intervals = await this.getSessionIntervals(sessionKey);
      ...
      const progressiveGaps = driverIntervals.map((interval, index) => {
        ...
        return {
          ...
          gapChange:
            previousInterval &&
            interval.gapToLeader !== null &&
            previousInterval.gapToLeader !== null
      ...
      this.logger.log(
        `Retrieved ${progressiveGaps.length} gap measurements for driver ${driverNumber}`,
      );
      return {
        sessionKey,
        driverNumber,
  ...
  async getIntervalsHistory(
    ...
    try {
      ...
      this.logger.log(
        `Generated intervals history with ${progression.length} time points`,
      );
      return {
        sessionKey,
        dateRange: {
          start:
            startDate || (intervals.length > 0 ? intervals[0].timestamp : null),
          end:
            endDate ||
            (intervals.length > 0
              ? intervals[intervals.length - 1].timestamp
              : null),
        },
        progression,
  ...
  private calculateTrend(intervals: any[], currentIndex: number) {
    ...
    const recentIntervals = intervals.slice(
