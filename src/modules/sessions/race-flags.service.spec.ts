...
describe('RaceFlagsService', () => {
  ...
  it.each([
    ...
    ['Practice 1', 'PRACTICE'],
  ])('getRaceFlags: session_type=%s → sessionType=%s', async (openF1Type, expected) => {
    mockClient.fetchSessions.mockResolvedValue([makeSession(openF1Type)] as any);
  ...
  it('getRaceFlags: 레이스 세션에서 플래그 이벤트 없으면 모든 랩이 NONE이다', async () => {
    mockClient.fetchSessions.mockResolvedValue([makeSession('Race')] as any);
    mockClient.fetchRaceControl.mockResolvedValue([]);
    mockClient.fetchLaps.mockResolvedValue(makeLaps(5) as any);
  ...
  it('getRaceFlags: SC 이벤트가 랩3에 발생하면 랩3 이후 SC, GREEN 이후 NONE', async () => {
    mockClient.fetchSessions.mockResolvedValue([makeSession('Race')] as any);
    mockClient.fetchLaps.mockResolvedValue(makeLaps(6) as any);
    mockClient.fetchRaceControl.mockResolvedValue([
      makeRcMsg({ date: '2024-04-07T04:10:00.000Z', lap_number: 3, message: 'SAFETY CAR DEPLOYED', category: 'SafetyCar' }),
      makeRcMsg({ date: '2024-04-07T04:15:00.000Z', lap_number: 5, flag: 'GREEN', message: 'GREEN FLAG' }),
    ]);
  ...
  it('getRaceFlags: RED FLAG 이벤트를 올바르게 분류한다', async () => {
    mockClient.fetchSessions.mockResolvedValue([makeSession('Race')] as any);
    mockClient.fetchLaps.mockResolvedValue(makeLaps(3) as any);
    mockClient.fetchRaceControl.mockResolvedValue([
      makeRcMsg({ date: '2024-04-07T04:08:00.000Z', lap_number: 2, flag: 'RED', message: 'RED FLAG' }),
    ]);
  ...
  it('getRaceFlags: VSC 이벤트를 올바르게 분류한다', async () => {
    ...
    mockClient.fetchLaps.mockResolvedValue(makeLaps(3) as any);
    mockClient.fetchRaceControl.mockResolvedValue([
      makeRcMsg({ date: '2024-04-07T04:08:00.000Z', lap_number: 1, message: 'VIRTUAL SAFETY CAR DEPLOYED' }),
    ]);
  ...
  it('getRaceFlags: scope가 Track이 아닌 메시지는 무시한다', async () => {
    ...
    mockClient.fetchLaps.mockResolvedValue(makeLaps(3) as any);
    mockClient.fetchRaceControl.mockResolvedValue([
      makeRcMsg({ date: '2024-04-07T04:08:00.000Z', lap_number: 1, flag: 'YELLOW', message: 'YELLOW FLAG', scope: 'Sector' }),
    ]);
  ...
  it('getRaceFlags: Flag/SafetyCar가 아닌 카테고리는 무시한다', async () => {
    ...
    mockClient.fetchLaps.mockResolvedValue(makeLaps(3) as any);
    mockClient.fetchRaceControl.mockResolvedValue([
      makeRcMsg({ date: '2024-04-07T04:08:00.000Z', lap_number: 1, message: 'SAFETY CAR', category: 'Other', scope: 'Track' }),
    ]);
  ...
  it('getRaceFlags: 퀄리파잉 세션에서 minuteFlags를 분 단위로 생성한다', async () => {
    mockClient.fetchSessions.mockResolvedValue([makeSession('Qualifying')] as any);
    mockClient.fetchLaps.mockResolvedValue(makeLaps(5) as any);
    mockClient.fetchRaceControl.mockResolvedValue([
      // 세션 시작(04:00)으로부터 30분 경과 시점에 RED FLAG
      makeRcMsg({ date: '2024-04-07T04:30:00.000Z', flag: 'RED', message: 'RED FLAG' }),
    ]);
