...
2. intervals를 2초 단위로 그룹화하여 프레임 생성
3. 각 프레임의 `currentLapTime`, `bestLapTime`, `miniSector`는 **이전 완료 랩** 데이터 표시 (`displayDataLap = currentLap - 1`)
4. `tireInfo`는 **현재 랩** 스틴트 데이터 사용
5. 결과는 인메모리 캐시(10분)에 저장 — 동일 세션 재요청 시 즉시 반환
...
| `interval` | `string` | 리더 기준 갭 (리더는 `"--"`, 나머지는 `"+1.234"`) |
| `intervalToAhead` | `string` | 바로 앞 차와의 갭 (리더는 `""`) |
...
```typescript
isDNF = lap_duration === null && !is_pit_out_lap && lapNumber > 1
```
